            ...
            <tbody>
                ...
                @forelse ($invoices as $invoice)
                    <tr class="{{ $loop->iteration % 2 == 0 ? 'bg-gray-100' : '' }}">
                        <td class="text-center font-bold">{{ $invoice->id }}</td>
                        <td class="text-center font-bold">{{ $invoice->user ? $invoice->user->name : 'N/A' }}</td>
                        <td class="text-center ">{{ $invoice->created_at }}</td>
                        <td class="text-center ">{{ $invoice->products->sum('pivot.quantity') }}</td>
                        <td class="text-center ">{{ $invoice->total . ' EGP' }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="5">
